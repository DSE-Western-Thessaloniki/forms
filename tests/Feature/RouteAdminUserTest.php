...
it('can create another user as admin', function (): void {
    ...
    $response->assertSessionHas('status', 'Ο χρήστης αποθηκεύτηκε!');
});
...
it('can delete another user as admin', function (): void {
    ...
    $response->assertStatus(302);
    $response->assertSessionHas('status', 'Ο χρήστης διαγράφηκε!');
});
...
it('cannot login with deactivated account', function (): void {
    ...
    $response->assertSessionHas('error', 'Ο λογαριασμός σας είναι απενεργοποιημένος.');
});
