...
it('can access the lists panel as admin with filter', function () {
    ...
    $response = $this->actingAs($admin)->call('GET', '/admin/selection_list', [
        'filter' => $lists[0]->name,
    ])->assertOk();

    $response->assertSee($lists[0]->name);
...
it('cannot create a copy of a list as user', function () {
    ...
    $this->actingAs($user)->post("/admin/selection_list/{$list->id}/copy")->assertForbidden();
    $this->assertDatabaseMissing('selection_lists', [
...
it('can create a copy of a list as author', function () {
    ...
    $this->actingAs($author)->post("/admin/selection_list/{$list->id}/copy")->assertRedirect('/admin/selection_list');
    $this->assertDatabaseHas('selection_lists', [
        'name' => $list->name.' (Αντίγραφο)',
        'active' => 1,
        'data' => '[]',
        'created_by' => $author->id,
...
it('can create a copy of a list as admin', function () {
    ...
    $this->actingAs($admin)->post("/admin/selection_list/{$list->id}/copy")->assertRedirect('/admin/selection_list');
    $this->assertDatabaseHas('selection_lists', [
        'name' => $list->name.' (Αντίγραφο)',
        'active' => 1,
        'data' => '[]',
